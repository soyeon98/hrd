...
export default function SignUpComponent({openPopupDaumPostApi,addr,회원가입,timer,timerCounterFn,mapAddressFn,confirmModalOpen}) { //내부 props와 외부 props가 섞이지 않도록 비구조화해서 넣어준다
        ...
        if(regExp2.test(value)===false || regExp3.test(value)===true|| regExp4.test(value)===false || regExp5.test(value)===true ){
            ...
            .then((res)=>{
                ...
                if(result.includes(true)){
                    ...
                    confirmModalOpen('사용 불가능한 아이디입니다.');
                }
                else{
                    setState({
                        ...state,
                    ...
                    confirmModalOpen('사용 가능한 아이디입니다.');
                }
            })
            .catch((err)=>{
        ...
        const onSubmitSignUpEvent=(e)=>{
          e.preventDefault();
            ...
            if(state.아이디===''){
                ...
                confirmModalOpen('아이디를 형식에 맞게 입력하세요');
            }
            else if(state.isIdOk===false){
                confirmModalOpen('아이디중복확인을 해주세요');
            }
            else if(state.비밀번호===''){
                ...
                confirmModalOpen('비밀번호를 형식에 맞게 입력하세요');
            }
            else if(state.비밀번호확인===''){
                confirmModalOpen('비밀번호확인을 다시 해주세요');
            }
                ...
                confirmModalOpen('이메일를 입력하세요');
            }
            else if(state.isEmailError===true){
                confirmModalOpen('이메일 형식을 확인하세요');
            }
            else if(state.isEmailOk===false){
                confirmModalOpen('이메일 중복확인을 해주세요');
            }
            else if(state.휴대폰===''){
                confirmModalOpen('휴대폰번호를 입력하세요');
            }
            else if(state.isHp3===true){
                confirmModalOpen('인증번호 확인을 해주세요');
            }
            else if(state.주소1===''){
                ...
                confirmModalOpen('필수 항목을 모두 체크해주셔야합니다.');
            }
            else{
            // 휴대폰 표현형식
            // 시작 숫자 3자리 $1
            ...
            //     "user_ph" :state.휴대폰.replace(regExpHp,'$1-$2-$3'),
            //     "user_address" :`${state.주소1} ${state.주소2}`,
            //     "user_gender" :state.성별,
            //     "user_birth" : `${state.생년}-${state.생월}-${state.생일}`,
            //     "user_chooga" : `${state.추가입력사항} ${state.참여이벤트명} ${state.추천인아이디}`,
            //     "user_service" : 약관동의               
            // }

            // 폼데이터를 파라미터데이터로 속성을 생성
            ...
            formData.append("user_ph" ,state.휴대폰.replace(regExpHp,'$1-$2-$3'));
            formData.append("user_address" ,`${state.주소1} ${state.주소2}`);
            formData.append("user_gender" ,state.성별);
            formData.append("user_birth" ,`${state.생년}-${state.생월}-${state.생일}`);
            formData.append("user_chooga" ,`${state.추가입력사항} ${state.참여이벤트명} ${state.추천인아이디}`);
            ...
            // http://192.168.40.28:8080/jsp/0512_NVC_MODELING_AJAX_DTO_DAO/쇼핑몰고객관리/kurly_user_signup_action.jsp
            // http://localhost:8080/jsp/0512_NVC_MODELING_AJAX_DTO_DAO/쇼핑몰고객관리/kurly_user_signup_action.jsp
            // http://127.0.0.1:8080/jsp/0512_NVC_MODELING_AJAX_DTO_DAO/쇼핑몰고객관리/kurly_user_signup_action.jsp
            // $.ajax({
            //     //url:'http://localhost:8080/jsp/0512_NVC_MODELING_AJAX_DTO_DAO/쇼핑몰고객관리/kurly_user_signup_action_test_parameter.jsp',
            //     url:'http://localhost:8080/jsp/0512_NVC_MODELING_AJAX_DTO_DAO/쇼핑몰고객관리/kurly_user_signup_action.jsp',
            //     type:'POST',
            //     data:formData,
            //     success(res){
            //         console.log('AJAX 성공');
            ...
            //         // 홈 인트로 페이지로 이동
            //         window.location.pathname="/메인페이지";
            //     },
            //     error(err){
            //         console.log('AJAX 실패!'+err);
            ...
            // [3] FETCH()
            // fetch('/jsp/0516_NVC_MODELING_AXIOS_DTO_DAO/쇼핑몰고객관리/kurly_user_signup_action.jsp',{
            //     method:'post',
            //     body:formData
            // })
            // .then((res)=>res.json())
            ...
            axios.get('/jsp/0516_NVC_MODELING_AXIOS_DTO_DAO/쇼핑몰고객관리/kurly_user_signup_action.jsp',{
                params:formData
            })
            .then((res)=>{
            ...
            //                          2. data : {}, new URLSearchParams();
            // 프로미스 반드시 ES6 사용 : 화살표함수 사용
            //async => await
            //AXIOS()  FETCH()

            // axios({
            //     url:'jsp/0516_NVC_MODELING_AXIOS_DTO_DAO/쇼핑몰고객관리/kurly_user_signup_action.jsp',
            //     method:'POST',
            //     data: {},
            //     params:formData
            // })
