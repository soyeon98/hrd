...
export default function CartComponent ({openPopupDaumPostApi,addr,confirmModalOpen}) {
    ...
    //체크박스 이벤트 구현
    const [isCheckAll,setiIsCheckAll] = React.useState(false); // 모두 선택
    const [isSelectDel,setIsSelectDel] = React.useState(false); // 선택 삭제
    ...
    const initMethod=()=>{
        if(localStorage.getItem('SY_KURLY_CART_PRODUCT')!==null){
            ...
            // 임시배열변수에 저장된 배열값을 모두 checkAll[]에 저장
            setCheckAll(imsi);
                        ...
                        <div className="content">
                            <div className="left">
                                <ul>
                                    <li>  {/* 위 쪽 전체 선택 삭제 */}
                                        <span>
                                            <button onClick={onClickCheckAll} className={`select-btn ${isCheckAll?' on':''}`}>전체선택
                                                <strong>(
                                                    <em className='select-count'>{check.length}</em>/<em className='cart-tot'>{checkAll.length}</em>
                                                    )</strong>
                                            </button>
                                            <i>|</i>
                                            <button onClick={onClickSelectDelete} disabled={!isSelectDel} className={`delete-btn ${isSelectDel?' on':''}`}>선택삭제</button>
                                        </span>
                                    </li>
                                                    ...
                                                    (
                                                        <div>
                                                            <dl ref={(el)=>(ddRef.current[0]=el)}>
                                                                <dt>
                                                                    <div><img className={!isUp[0]?'on':''} src="./images/cart/cart_icon1.svg" alt="" /><h3>냉장 상품</h3></div>
                                                                    <div><button onClick={(e)=>onClickSlideUpDown(e,0)}><img className={!isUp[0]?'on':''} src="./images/cart/arrow_up.svg" alt="" /></button></div>
                                                                </dt>
                                                                {res1}
                                                            </dl>
                                                        </div>
                                                    ...
                                                    (
                                                        <div>
                                                            <dl ref={(el)=>(ddRef.current[1]=el)}>
                                                                <dt>
                                                                    <div><img className={!isUp[1]?'on':''} src="./images/cart/cart_icon2.svg" alt="" /><h3>냉동 상품</h3></div>
                                                                    <div><button onClick={(e)=>onClickSlideUpDown(e,1)} ><img className={!isUp[1]?'on':''} src="./images/cart/arrow_up.svg" alt="" /></button></div>
                                                                </dt>
                                                                {res2}
                                                            </dl>
                                                        </div>
                                                        ...
                                                        (
                                                            <div>
                                                                <dl ref={(el)=>(ddRef.current[2]=el)}>
                                                                    <dt>
                                                                        <div><img className={!isUp[2]?'on':''} src="./images/cart/cart_icon3.svg" alt="" /><h3>상온 상품</h3></div>
                                                                        <div><button onClick={(e)=>onClickSlideUpDown(e,2)} ><img className={!isUp[2]?'on':''} src="./images/cart/arrow_up.svg" alt="" /></button></div>
                                                                    </dt>
                                                                    {res3}
                                                                </dl>
                                                            </div>
